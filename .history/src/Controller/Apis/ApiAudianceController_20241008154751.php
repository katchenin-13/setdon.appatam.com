...
#[Route('/api/audience')]
class ApiAudienceController extends ApiInterface
{
    #[Route('/', name: 'api_audeince', methods: ['GET'])]
    #[OA\Response(
        response: 200,
    ...
    public function getAll(AudienceRepository $audeinceRepository): Response
    {
        try {
            $demandes = $audeinceRepository->findAll();
            $tabaDemande = [];
            $i = 0;
    ...
    public function justification(Request $request, AudienceRepository $audeinceRepository, $id)
    {
        try {
            $data = json_decode($request->getContent());

            $audience = $audeinceRepository->find($id);
            if ($audience != null) {
                ...
                $audeinceRepository->save($audience, true);
